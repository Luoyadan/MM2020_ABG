...
class NodeUpdateNetwork(nn.Module):
	...
	def forward(self, node_source, node_target, source_edge, target_edge):

		# compute attention and aggregate
		source_aggr_feat = torch.mm(source_edge, node_target)
		target_aggr_feat = torch.mm(target_edge, node_source)

		source_node_feat = torch.cat([node_source, source_aggr_feat], -1).unsqueeze(0).transpose(1, 2)
		target_node_feat = torch.cat([target_aggr_feat, node_target], -1).unsqueeze(0).transpose(1, 2)
...
class EdgeUpdateNetwork(nn.Module):
	...
	def forward(self, node_source, node_target, edge_source, edge_target):
		# compute abs(x_i, x_j)
		# num_tasks = node_feat.size(0)
		# num_data = node_feat.size(1)
		edge_source = F.normalize(edge_source.float(), p=1, dim=1)
		edge_target = F.normalize(edge_target.float(), p=1, dim=1)
		...
		sim_val = torch.sigmoid(self.sim_network(x_ij)).squeeze(0)
		edge_feat = sim_val + 1e-6
		edge_feat = edge_feat.squeeze()
		# if edge_target.size(1) != edge_target.size(0) or edge_target.size(1)!=edge_source.size(0):
		# 	print("target and source edge size do not match")
		source_edge_feat = edge_source.mm(edge_feat.t()).mm(edge_target.t()) #target to source
		target_edge_feat = edge_target.mm(edge_feat).mm(edge_source.t())
		return source_edge_feat, target_edge_feat

class GraphNetwork(nn.Module):
	def __init__(self, in_features, node_features, edge_features, num_layers, dropout=0.5):
		...
		print('finished constructing GNN')

	# forward
	def forward(self, feat_base_source, feat_base_target, source_edge_frame, target_edge_frame):
		...
		for l in range(self.num_layers):
			# (1) edge update
			source_edge, target_edge = self._modules['node2edge_net{}'.format(l)](node_source, node_target, source_edge_frame, target_edge_frame)
			# (2) node update
			node_source, node_target = self._modules['edge2node_net{}'.format(l)](node_source, node_target, source_edge, target_edge)

			# save edge feature
			edge_feat_list.append(source_edge)
...
class VideoModel(nn.Module):
	...
	def _prepare_DA(self, num_class, base_model): # convert the model to DA framework
		...
		self.GNN = GraphNetwork(in_features=self.feature_dim, node_features=self.fc_dim, edge_features=self.fc_dim,
								num_layers=self.num_layers)


		# 3. domain feature layers (frame-level)
		self.fc_feature_domain = nn.Linear(self.feature_dim, feat_frame_dim)
	...
	def forward(self, input_source, input_target, source_label, beta, mu, is_train, reverse):
		...
		source_label_frame = source_label.unsqueeze(1).repeat(1, num_segments).view(-1) # expand the size for all the frames
		...
		edge_feat_list, node_source_feat_list, node_target_feat_list = self.GNN(feat_base_source, feat_base_target,
																				source_edge_frame.cuda(), target_edge_frame.cuda())

		# === adversarial branch (frame-level) ===#
		pred_fc_domain_frame_source = self.domain_classifier_frame(feat_base_source, beta)
		pred_fc_domain_frame_target = self.domain_classifier_frame(feat_base_target, beta)
		...
		for layer in range(self.num_layers):
			...
			feat_fc_source = node_source_feat_list[layer]
			feat_fc_target = node_target_feat_list[layer]
			...
			out_source_layer.append(output_source)
			out_target_layer.append(output_target)



		return attn_relation_source, out_source_layer, output_source_2, pred_domain_all_source[::-1], feat_all_source[::-1], attn_relation_target, out_target_layer, output_target_2, pred_domain_all_target[::-1], feat_all_target[::-1] # reverse the order of feature list due to some multi-gpu issues
