...
def main():
	...
	model = VideoModel(num_class, args.baseline_type, args.frame_aggregation, args.modality,
				...
				verbose=args.verbose, share_params=args.share_params, num_layers=args.num_layers)
	...
	if not args.evaluate:
		...
		source_loader = torch.utils.data.DataLoader(source_set, batch_size=args.batch_size[0], shuffle=False, sampler=source_sampler, num_workers=args.workers, drop_last=True, pin_memory=True)
		...
		target_loader = torch.utils.data.DataLoader(target_set, batch_size=args.batch_size[1], shuffle=False, sampler=target_sampler, num_workers=args.workers, drop_last=True, pin_memory=True)
...
def train(num_class, source_loader, target_loader, model, criterion, criterion_domain, optimizer, epoch, log, log_short, alpha, beta, gamma, mu):
	...
	for i, ((source_data, source_label),(target_data, target_label)) in data_loader:
		...
		if batch_source_ori < args.batch_size[0] or batch_target_ori < args.batch_size[1]:
			continue
		...
		#====== forward pass data ======#
		attn_source, out_source, out_source_2, pred_domain_source, feat_source, attn_target, out_target, out_target_2, \
		pred_domain_target, feat_target = model(source_data, target_data, source_label, beta,
												mu, is_train=True, reverse=False)
		...
		label = label_source
		loss = criterion(out_source[-1], label)
		losses_c.update(criterion(out_source[-1], label).item(), out_source[-1].size(0))

		for layer in range(args.num_layers - 1):
			out = out_source[layer]
			loss_classification = criterion(out, label)
			loss += loss_classification * 0.5
		...
		if args.adv_DA != 'none' and args.use_target != 'none':
			...
			for l in range(1):
				...
				loss_adversarial += loss_adversarial_single * 3.5
		...
		top1.update(prec1.item(), out_source[-1].size(0))
		top5.update(prec5.item(), out_source[-1].size(0))
...
def validate(val_loader, model, criterion, num_class, epoch, log):
	...
	for i, (val_data, val_label) in enumerate(val_loader):
		...
		with torch.no_grad():
			...
			# compute output
			_, _, _, _, _, attn_val, out_val, out_val_2, pred_domain_val, feat_val = model(val_data, val_data, val_label, [0]*len(args.beta), 0, is_train=False, reverse=False)
			...
			loss = criterion(out_val[-1], label)
			...
			prec1, prec5 = accuracy(out_val[-1].data, label, topk=(1, 5))

			losses.update(loss.item(), out_val[-1].size(0))
			top1.update(prec1.item(), out_val[-1].size(0))
			top5.update(prec5.item(), out_val[-1].size(0))
