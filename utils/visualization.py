...
def visualize_TSNE(feat, label, num_class, args, split):
    ...
    label = (label < num_class-1).astype(np.int).reshape(-1)
    ...
    # source_feat_select = source_feat[np.random.choice(source_feat.size(0), int(source_feat.size(0)/10))]
    src_label = np.full([source_feat.shape[0]], 2)


    X = np.vstack(target_feat[ind])
    X = np.concatenate([X, source_feat])
    y = np.hstack(label[ind])
    y = np.concatenate([y, src_label])

    digits_proj = TSNE(random_state=9).fit_transform(X)
