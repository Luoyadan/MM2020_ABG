...
def train(num_class, source_loader, target_loader, model, criterion, criterion_domain, optimizer, epoch, log, log_short, alpha, beta, gamma, mu):
	...
	for i, ((source_data, source_label),(target_data, target_label)) in data_loader:
		...
		pred_domain_target, feat_target, source_to_target_edge, high_order_edge_map = model(source_data, target_data, source_label, beta, mu, is_train=True, reverse=False)
		...
		elif args.baseline_type == 'frame':
			attn_source, out_source, out_source_2, pred_domain_source, feat_source = removeDummy(attn_source,
																								 out_source,
																								 out_source_2,
																								 pred_domain_source,
																								 feat_source,
																								 batch_source_ori * args.num_segments)
			attn_target, out_target, out_target_2, pred_domain_target, feat_target = removeDummy(attn_target,
																								 out_target,
																								 out_target_2,
																								 pred_domain_target,
																								 feat_target,
																								 batch_target_ori * args.num_segments)
		...
		if i % args.print_freq == 0:
			...
			line = line.format(
				...
				loss=losses, loss_c=losses_c, loss_d=losses_d, loss_a=losses_a, loss_e=losses_e, loss_s=losses_s, loss_edge=losses_edge,
		...
		with torch.no_grad():
			...
			if args.baseline_type == 'video':
				...
				attn_val, out_val, out_val_2, pred_domain_val, feat_val = removeDummy(attn_val, out_val, out_val_2,
																					  ...
																					  batch_val_ori * args.val_segments)
			# measure accuracy and record loss
